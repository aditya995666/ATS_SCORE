...
Original file is located at
    https://colab.research.google.com/drive/1hWv1a6B-ifMg1h0sVJSaXwxMCcXSOJsg
"""
...
def extract_resume_texts(folder_path):
    ...
    for file in resume_files:
        ...
        try:
            ...
            cleaned = ""
        resume_texts.append((file, cleaned))
...
def match_with_bert(resume_texts, job_desc_dict, top_n=3):
    job_titles = list(job_desc_dict.keys())
    job_texts = [clean_text(desc) for desc in job_desc_dict.values()]
    ...
    for filename, resume_text in resume_texts:
        ...
        if not resume_text.strip():
            print("⚠️ Empty or unreadable resume")
            continue
        ...
        for rank, idx in enumerate(top_indices, 1):
            ...
            print(f" {rank}. {title:<25} | Score: {score:.2f}% | {remark}")
            results.append({
                "Resume": filename,
                "Job Title": title,
                "Score": f"{score:.2f}",
    ...
    df.to_csv("ats_output.csv", index=False)
    print("\n📁 Results saved to ats_output.csv")

# ✅ Main
if __name__ == "__main__":
    print("🔍 Running Enhanced ATS with BERT...\n")
    resumes = extract_resume_texts(RESUME_FOLDER)
    if not resumes:
        print("⚠️ No PDF resumes found in 'resume/' folder.")
    else:
        match_with_bert(resumes, job_descriptions)
